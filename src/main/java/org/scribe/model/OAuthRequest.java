...
public class OAuthRequest extends Request
{
  ...
  {
      if (key.startsWith(OAUTH_PREFIX) || key.equals(OAuthConstants.SCOPE) || key.equals(OAuthConstants.STATE) || key.equals(OAuthConstants.REALM))
    {
      return key;
    }
    else
    {
        throw new IllegalArgumentException(String.format("OAuth parameters must either be '%s', '%s', '%s' or start with '%s'", OAuthConstants.SCOPE, OAuthConstants.STATE, OAuthConstants.REALM, OAUTH_PREFIX));
    }
  }
